...
<script type="text/javascript">
      ...
      $.ajax({
        url: '<?=base_url()?>' + 'admin/get_famlist_data',
        success: function(data){
          ...
          var html = '<div class="x_content">'+
            ...
            $.each(obj, function(index, value){
              console.log(value);
              html += '<tr>'+'<th scope="row">'+i+'</th>'+'<td>'+value.id+'</td>'+'<td>'+value.first_name+''+value.last_name+'</td>'+'<td>'+value.email+'</td>'+'<td>'+value.area+'</td>'+'<td>'+value.city+'</td>'+'<td>'+value.tbm_count+'</td>'+'<td><a href="<?=base_url()?>admin/tbm_list_load/'+value.id+'"><button class="btn">Get TBM List</button><a></td>'+'</tr>';
              i+=1;
            })
            html += '</tbody>';
